...
from config import EDITION, OUTPUT_DIR, SENSITIVE_PORTS, SEVERITY, CONFIDENCE
...
<div class="header"><h1>PHANTOMRECON <span style="color:#4488ff;font-size:14px">Free Edition</span></h1><div class="target">{htmlmod.escape(target)}</div><div class="date">{now}</div></div>
...
<div class="footer">PhantomRecon Free Edition — <a href="https://github.com/Raphaellopes-dev/phantomrecon" target="_blank">github.com/Raphaellopes-dev/phantomrecon</a></div>
</div></body></html>"""
    ...
    txt = f"PHANTOMRECON Free Edition\n{'='*50}\nAlvo: {target}\nData: {now}\n{'='*50}\n\nIPs: {ips or 'N/A'}\n\n{score_txt}"
    if ports:
        txt += "PORTAS ABERTAS:\n" + "-"*40 + "\n"
        for p, s, b in ports:
